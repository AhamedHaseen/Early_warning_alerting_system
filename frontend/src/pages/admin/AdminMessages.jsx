...
import { Send, Users, User, Shield, Search, Paperclip, MoreVertical, MessageSquare, GraduationCap } from 'lucide-react';
...
const AdminMessages = () => {
  const { user } = useAuth();
  const [recipientType, setRecipientType] = useState('student'); // 'student', 'lecturer', 'batch'
  ...
  useEffect(() => {
    if (activeChat && user && activeChat.role) {
      ...
      // It's a batch, we don't fetch message history natively for broadcasts
      setMessages([]);
    }
  }, [activeChat, user]);
  ...
  const fetchData = async () => {
    try {
      const [profilesRes, batchesRes, unreadRes] = await Promise.all([
        supabase.from('profiles').select('id, full_name, role').in('role', ['student', 'lecturer']).order('full_name'),
    ...
    if (activeChat.year) {
      try {
        ...
        const msgs = targetStudentIds.map(studentId => ({
          ...
          content: `[Broadcast from Admin to ${activeChat.name}] ${newMessage.trim()}`
        }));
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex animate-in fade-in slide-in-from-bottom-4 duration-500 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      ...
      <div className="w-80 border-r border-slate-100 flex flex-col bg-slate-50/30">
        ...
        <div className="flex p-2 space-x-1 border-b border-slate-100 overflow-x-auto">
          <button 
            onClick={() => setRecipientType('student')}
            className={`flex-1 py-1.5 px-2 text-xs font-medium rounded-md transition-colors whitespace-nowrap ${recipientType === 'student' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-600 hover:bg-slate-100'}`}
          >
            Students
          </button>
          <button 
            onClick={() => setRecipientType('lecturer')}
            className={`flex-1 py-1.5 px-2 text-xs font-medium rounded-md transition-colors whitespace-nowrap ${recipientType === 'lecturer' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-600 hover:bg-slate-100'}`}
          >
            Lecturers
          </button>
          <button 
            onClick={() => setRecipientType('batch')}
            className={`flex-1 py-1.5 px-2 text-xs font-medium rounded-md transition-colors whitespace-nowrap ${recipientType === 'batch' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-600 hover:bg-slate-100'}`}
          >
            Batches
          </button>
        </div>
            ...
            filteredItems.map(item => (
              ...
              >
                <div className="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center text-slate-500 shrink-0">
                  {recipientType === 'student' && <GraduationCap className="w-5 h-5" />}
                  {recipientType === 'lecturer' && <User className="w-5 h-5" />}
      ...
      {activeChat ? (
          ...
          <div className="h-16 border-b border-slate-100 flex items-center justify-between px-6 bg-slate-50/50">
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center text-slate-500">
                {activeChat.role === 'student' && <GraduationCap className="w-5 h-5" />}
          ...
          <div className="flex-1 p-6 overflow-y-auto bg-slate-50/30">
            {activeChat.year ? (
               <div className="flex h-full items-center justify-center flex-col text-slate-400">
                  ...
                  <p className="text-sm font-medium">Admin Broadcast Mode</p>
        ...
        <div className="flex-1 flex flex-col items-center justify-center bg-slate-50">
          ...
          <h2 className="text-xl font-semibold text-slate-800">Admin Messages</h2>
          <p className="text-slate-500 mt-2 text-sm text-center max-w-sm">Select a user or batch from the sidebar to start messaging or create a new one.</p>
        </div>
      )}
